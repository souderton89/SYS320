...
read courseinfo 
echo ""
echo "Courses in $courseinfo"
cat "$courseFile" | grep "$courseinfo" | cut -d';' -f1,2,5,6,7 | \
sed 's/;/ | /g'
echo ""
...
read subjectinfo 
  echo ""
  echo "Subject in $courseinfo"
  cat "$courseFile" | grep "$subjectinfo" | cut -d';' -f1,2,5,6,7,8,9,10,11,12 | \
  sed 's/;/ | /g'
  echo ""
